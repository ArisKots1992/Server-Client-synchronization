...
#define MD5_memcpy(d, s, n) memcpy(d, s, n)
#define MD5_memset(s, c, n) memset(s, c, n)
...
void MD5InitIV(MD5_CTX *context, unsigned int digest[4], int count) {
  // "count" must fit in an int.
  context->count[0] = count*8;
  context->count[1] = 0;
  /* Load magic initialization constants. */
  context->state[0] = digest[0];
  context->state[1] = digest[1];
  context->state[2] = digest[2];
  context->state[3] = digest[3];
}
...
static inline void MD5Transform(UINT4 state[4], unsigned char block[64]) {
  ...
  // Assumes little endian machine and that block is always UINT4 aligned.
  UINT4 *x = (UINT4 *)block;
 ...
 */
void MD5Final(unsigned int digest[4], MD5_CTX *context) {
  unsigned char bits[8];
  unsigned int index, padLen;

  /* Save number of bits */
  //  Encode (bits, context->count, 8);
  *(UINT4 *)bits = context->count[0];
  *(UINT4 *)(bits+4) = context->count[1];
...
// Convenience method added for project debug purposes
void MD5Print( unsigned int *mdContext )
{
	int i;
	for( i = 0 ; i < 16 ; ++i )
		printf("%02x", ((unsigned char*)mdContext)[i]);
}
